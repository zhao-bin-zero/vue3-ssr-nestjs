...
class SsrServer {
	...
	constructor() {
		...
		this.vueTemplate = this.isProd ? readFileSync(this.resolve('dist/web/client/index.html'), 'utf-8') : readFileSync(this.resolve('index.html'), 'utf-8');
		this.manifest = this.isProd ? require('./dist/web/client/ssr-manifest.json') : {};
	...
	async init(app) {
		if (!this.isProd) {
			...
			app.use(
				require('serve-static')(this.resolve('dist/web/client'), {
					index: false
				})
			);
		}
